...
class RegistryResource(BaseModel):
    ...
    id: str = Field(..., description="Enterprise semantic model field description")
    type: str = Field(
        ...,
        description="Phân loại: CAPABILITY, SPECIFICATION, WORKFLOW, AGENT...",
    )
    name: str = Field(..., description="Enterprise semantic model field description")
    metadata: dict[str, Any] = Field(default_factory=dict, description="Enterprise semantic model field description")

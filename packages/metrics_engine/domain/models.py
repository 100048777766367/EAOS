...
class ArchitectureHealthAggregate:
    ...
    def calculate_mttr(self) -> float:
        ...
        mttr_obs = [
            ...
            if o.metric_type == MetricType.MTTR_MINUTES
        ]
        if not mttr_obs:
            return 0.0
    ...
    def calculate_ai_success_rate(self) -> float:
        ...
        ai_obs = [
            o.value
            for o in self.observations
            if o.metric_type == MetricType.AI_SUCCESS_RATE
        ]
        if not ai_obs:
    ...
    def calculate_architecture_drift_index(self) -> float:
        ...
        drift_obs = [
            o.value
            for o in self.observations
            if o.metric_type == MetricType.ARCHITECTURE_DRIFT_INDEX
        ]
        if not drift_obs:
            return 0.0
    ...
    def evaluate_capability_health(
        ...
        cap_obs = [
            o for o in self.observations if o.target_component == capability_id
        ]

        violations = sum(
            int(o.value)
            for o in cap_obs
            if o.metric_type == MetricType.RULE_VIOLATION_COUNT
        )
        incidents = sum(
            int(o.value)
            for o in cap_obs
            if o.metric_type == MetricType.INCIDENT_TREND_RATE
        )
        drift_vals = [
            o.value
            for o in cap_obs
    ...
    def compute_overall_system_health(self) -> float:
        ...
        policy_violations = sum(
            o.value
            for o in self.observations
            if o.metric_type == MetricType.POLICY_VIOLATION_COUNT
        )
        ...
        penalty = (
            (rule_violations * 5.0)
            + (policy_violations * 5.0)
            + (drift_index * 40.0)
            + ((100.0 - ai_success) * 0.3)
        )

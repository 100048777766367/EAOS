"""EAOS Runtime Control Plane operational control engine."""
...
from runtime.automation.bounded_recovery import (
    BoundedRecoveryEngine,
    RecoveryOutcome,
)
from runtime.inventory.process_control import (
    ProcessManager,
    ProcessOwnershipDTO,
    ProcessState,
)
...
class RuntimeControlPlane:
    """Observe, correlate, control, and verify runtime execution."""
    ...
    def create_task(
        ...
        """Create a task with stable correlation context and lifecycle FSM."""
        ctx = self.correlation.create_context(
    ...
    def observe_system_graph(self) -> list[RuntimeSystemGraphNodeDTO]:
        """Build the real-time Runtime System Graph."""
        processes = self.process_manager.inspect_all_processes()
        api_proc = next(
            (
                process
                for process in processes
                if process.capability_name == "api_gateway"
            ),
            None,
        )
        ...
        return [
            RuntimeSystemGraphNodeDTO(
                ...
                layer="User",
                status="ACTIVE",
                details="Operator Request",
            ),
            RuntimeSystemGraphNodeDTO(
                node_id="node-ui",
                layer="UI",
                status="ACTIVE",
                details="FastAPI Dashboard / Agent Chat UI",
            ),
            RuntimeSystemGraphNodeDTO(
                node_id="node-http",
                layer="HTTP/API",
                status=api_status,
                details="Port 8000 Gateway",
            ),
            RuntimeSystemGraphNodeDTO(
                node_id="node-ws",
                layer="WebSocket",
                status=api_status,
                details="Route /ws/chat",
            ),
            RuntimeSystemGraphNodeDTO(
                node_id="node-app",
                layer="Application",
                status="ACTIVE",
                details="EAOS Domain Services",
            ),
            RuntimeSystemGraphNodeDTO(
                node_id="node-agent",
                layer="Agent Orchestrator",
                status="ACTIVE",
                details="ChatOrchestrator",
            ),
            RuntimeSystemGraphNodeDTO(
                node_id="node-process",
                layer="Process",
                status=api_status,
                details="Uvicorn / Python PID",
            ),
            RuntimeSystemGraphNodeDTO(
                node_id="node-mutation",
                layer="Repository Mutation",
                status="GUARDED",
                details="L0-L5 Bounded Autonomy",
            ),
            RuntimeSystemGraphNodeDTO(
                node_id="node-verification",
                layer="Verification",
                status="ACTIVE",
                details="EAOS Quality Gates",
            ),
        ]
    ...
    def get_control_status(self) -> RuntimeControlStatusDTO:
        ...
        all_healthy = all(
            process.state == ProcessState.HEALTHY
            for process in processes
            if process.capability_name == "api_gateway"
        )
    ...
    def handle_runtime_failure(
        ...
        failure = self.failure_classifier.classify_exception(
            exc,
            affected_component,
            task_id,
        )
        ...
        if self.correlation.get_context(task_id) is None:
            self.correlation.create_context(task_id=task_id)
